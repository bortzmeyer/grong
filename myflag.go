...
type allFlags struct {
	...
	offset int // Always 0 unless the client calls Reinit
}
...
func Arg(i int) string {
	i += flags.first_arg + flags.offset
...
func NArg() int { return len(os.Args) - flags.offset - flags.first_arg }

// Args returns the non-flag command-line arguments.
func Args() []string { return os.Args[flags.first_arg + flags.offset:] }

// LastOption is the index of the last option. Useful for Reinit()
func LastOption() int { return flags.first_arg - 1 }
...
func Parse() {
	for i := flags.offset+1; i < len(os.Args); {
...
// Reinitializes the package for a new parsing starting at argument where
func Reinit(where int) {
	flags = &allFlags{make(map[string]*Flag), make(map[string]*Flag), where, where}
}
...
func ResetForTesting(usage func()) {
	flags = &allFlags{make(map[string]*Flag), make(map[string]*Flag), 1, 0}
	Usage = usage
	panicOnError = true
}
